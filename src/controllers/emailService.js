...
const sendEmailAndSavePost = async (recipient, subject, body, res) => {
  ...
  try {
    ...
    res.json({
      message: "E-mail enviado e post salvo com sucesso",
      redirectTo: "/",
    });
  } catch (error) {
    console.error("Erro ao enviar o e-mail ou salvar o post:", error);
    res.status(500).json({
      error: "Erro ao enviar o e-mail ou salvar o post",
